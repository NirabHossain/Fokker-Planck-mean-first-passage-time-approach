...
\end{equation}
Taking the adjoint of this condition (multiplying by $T$, integrating and transferring derivatives via integration by parts) yields 
\[ D(0^+)\,T'{(0^+)}
-D(0^-)\,T'{(0^-)}
=\kappa\,T(0). \]
With the symmetricity and continuity of $D(x)$ at $x=0$, we have $T'(0^-) = -T'(0^+)$ by symmetry of $T$ and the adjoint flux-jump condition then simplifies to the \emph{Robin-like} condition
\begin{equation}
{\;
2\,D(0)\,T'(0^+) \;=\; \kappa\,T(0).
\;}\label{eq:robin}
