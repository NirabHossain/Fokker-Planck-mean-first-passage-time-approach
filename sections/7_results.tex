...
Several idealizations of the present model should be kept in mind when interpreting the results.  First, the $\delta$-sink at $x=0$ is a singular caricature of a real target: a physical food source has finite extent, and replacing the $\delta$ by a smooth absorbing region of width $\sigma$ would shift the dwell-time contribution $2\pi/\kappa$ by an $O(\sigma)$ correction and broaden the trap-localised gradient in $T(x)$ that we have treated as a sharp Robin condition.  Second, the restriction to a periodic 1D ring is a strong geometric simplification; genuine nest interiors are 2D or 3D, and the analogue of the additive decomposition~\eqref{eq:T-avg-general} acquires geometric prefactors set by the spatial dimension and by the placement of the trap relative to the boundary. Third, the slowly-varying-$D$ regime adopted throughout excludes strongly anomalous subdiffusion ($\langle x^2\rangle\sim t^\alpha$ with $\alpha<1$), which would invalidate the Markovian Fokker--Planck description and can render $\langle T\rangle$ infinite~\cite{Metzler2000}.  These caveats bound the immediate biological reach of the results and motivate the extensions below.\\
