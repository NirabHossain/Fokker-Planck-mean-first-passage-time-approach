...
depends on the \emph{departure} site.  In symmetric exclusion (SEP), volume
exclusion from the crowders restricts the \emph{arrival} site instead: a particle
...
=\int_{-\pi}^{\pi} f\,(Dg')'\;dx
=\underbrace{[f\,Dg']_{-\pi}^{\pi}}_{\substack{=0\\\text{periodicity}}}
-\int_{-\pi}^{\pi} f'\,D\,g'\;dx,
\]
...
\item coefficient of $p$: $-\mu'+\tfrac12(\sigma^2)''=-D''+D''=0$ \checkmark.
\end{itemize}
The It\^o SDE corresponding to Fick's form is therefore
\begin{equation}
\boxed{\;
