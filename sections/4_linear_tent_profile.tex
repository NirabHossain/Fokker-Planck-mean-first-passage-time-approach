...
Figure~\ref{fig:tent-profile}. The profile is continuous but not differentiable at
$x=0$.
...
of $D_1$ ($D_0=1$).  %Positive $D_1$ produces a peak at the trap $x=0$; negative $D_1$ produces a dip.  
}
...
obtained by substituting $D(y)\equiv D_0$ into \eqref{eq:T-final}.
\end{enumerate}
...
\]
Substituting $u=\pi-y$ (so $y=\pi-u$, $D(y)=D_0+D_1 u/\pi$, $dy=-du$),
\[
